...
function ignoreFiles(rawFiles, ignoreList) {
    const filteredFiles = micromatch_1.default(rawFiles, ignoreList, {
        dot: true
    });
    const finalFiles = [];
...
function markdown2text(markdown) {
...
exports.markdown2text = markdown2text;
/**
 ...
 */
function gatherToxicSentences(label, results, sentences) {
    let isToxic = false;
    const toxicSentences = [];
    for (let i = 0; i < results.length; ++i) {
        if (results[i].match) {
            isToxic = true;
            const toxicSentence = sentences[i];
...
exports.toxicityClassification2paragraph = toxicityClassification2paragraph;
