...
const filewtf_1 = __importDefault(require("filewtf"));
...
function listFiles(rootDir) {
    const fileList = filewtf_1.default.walkthrough(rootDir);
    return fileList;
}
...
function getReadmeIgnoreList(filename) {
    ...
    const ignoreContent = readFileContent(filename);
    const ignoreList = ignoreContent.split("\n");
    let sanitizedIgnoreList = [];
...
function getLintFileList(workspaceDir) {
    ...
    const workspaceFiles = micromatch_1.default(rawWorkspaceFiles, ignoreList);
